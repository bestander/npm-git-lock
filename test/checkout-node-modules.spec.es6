...
describe(`npm-git-lock`, function() {
    ...
    beforeEach(() => {
        ...
        execSync(`git add .`);
        execSync(`git commit --author="npm-git-lock <bob@example.com>" -a -m "first commit" `);
    ...
    it(`should checkout node_modules from remote repo resetting all local changes`, function(done) {
        ...
        execSync(`git add .`);
        execSync(`git commit --author="npm-git-lock <bob@example.com>" -a -m "node_modules is cached"`);
        let packageJsonSha1 = crypto.createHash(`sha1`).update(packageJson).digest(`base64`);
        ...
        execSync(`git add .`);
        execSync(`git commit --author="npm-git-lock <bob@example.com>" -a -m "another commit that should be ignored" `);
